...
def aprox(u0,v0,dt,ds,dx,p,sn,xn,tn,expr1,s,x,w,xo,k0):
    ...
    for k in range(1,len(tn)):
        ruido = np.random.normal(loc=0, scale=1, size=len(xn))
        un=u0.copy()
        vn=v0.copy()
        A.append(un)
        B.append(vn)
...
###
        ...
        for i in range(0,len(xn)-1):
            #vn[i]=(1-dt)*v0[i]+summer3(v0,un[0],dx,xn,tn[k],i,w)*dt+np.sin(2*np.pi*5*t_now)
            
            vn[i]=(1-dt)*v0[i]+summer3(v0,un[0],dx,xn,tn[k],i,w)*dt+ruido[i]
