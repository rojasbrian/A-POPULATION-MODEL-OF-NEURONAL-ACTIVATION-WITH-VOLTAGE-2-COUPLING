...
for archivo in os.listdir(ruta):
    if archivo.endswith(".txt"):
        ...
        for i in range(0,len(Nt)):
            ...
            t=t+dt
        print(len(N),len(tn))    
        for p in ["f1","fe5","f5","R","iseX"]:
            if str(p) in archivo:
                if p=="f1":
                    name="frequency is 1"
                if p=="fe5":
                    name="frequency is 0.5"
                if p=="f5":
                    name="frequency is 5"
                if p=="R":
                    name= "The function I follows a standard normal distribution N(0,1).."
                if p=="iseX":
                    name= "stochastic process"

                

        plt.plot(tn,N,label=str(name))
...
for archivo in os.listdir(ruta):
    if archivo.endswith(".txt"):
        ...
        plt.plot(f,PSD,label="PSD of " +str(name))
